 ...
 * written to {@code ./data/jcustos/bootstrap.token} by
...
public class SetupView extends Composite<Div>
  ...
  private void submit() {
    ...
    switch (result) {
      ...
      case InitialAdminCreationResult.InternalError(String reason, Throwable cause) -> {
        // reason is end-user-safe; cause is the diagnosis the service now
        // attaches (and itself logs at WARN). Consume the cause here for our
        // own log/diagnostics instead of guessing it — it is the real
        // throwable behind the failure (e.g. a persistence I/O error).
        if (cause != null) {
          logger().error("Setup failed with internal error: {}", reason, cause);
        } else {
          logger().warn("Setup failed with internal error: {} (no cause attached)", reason);
        }
  ...
  /**
   * Minimum password length, pulled from the active server-side
   * {@code PasswordPolicy} so the UI hint and the pre-check can never
   * drift from what the server enforces. Falls back to 1 if the policy
   * declares no minimum.
   */
