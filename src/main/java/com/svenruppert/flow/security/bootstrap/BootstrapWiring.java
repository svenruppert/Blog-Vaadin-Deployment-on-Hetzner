...
import eu.jsentinel.jcustos.bootstrap.PasswordPolicy;
 ...
 *       {@code ./data/jcustos/bootstrap.token} and prints both the
 ...
 *
 * <p>Delete {@code ./data/jcustos/bootstrap.token} and the
 * {@code ./data/app-store} application store (which holds the admin
 * account) to force a re-bootstrap.
 */
public final class BootstrapWiring implements HasLogger {
  ...
  /**
   * Minimum admin password length — the single seed for the password
   * policy. The UI hint and any pre-check pull the value back out of
   * {@link #policy()} ({@link PasswordPolicy#minLength()}) so the
   * server policy and the UI never drift.
   */
  ...
  /**
   * The active password policy — the single source of truth for the
   * minimum password length. UI hints read it via
   * {@code policy().minLength()} rather than duplicating the value.
   */
  ...
  private static BootstrapWiring build() {
    ...
    PasswordPolicy policy = new MinimumLengthPasswordPolicy(MIN_PASSWORD_LENGTH);
    ...
    log.info("Initial-admin bootstrap pipeline wired: hashing=BouncyCastle/modern (Argon2id), "
        ...
        policy.minLength().orElse(1));
    return new BootstrapWiring(state, service, policy);
  }
}
