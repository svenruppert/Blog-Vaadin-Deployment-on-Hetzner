 ...
 */
public class JCustosBootstrapInitListener
  ...
  public void serviceInit(ServiceInitEvent event) {
    ...
    if (authn == null || authz == null) {
      ...
      // every request silently bypasses jCustos, which is the worst
      ...
      String diagnosis = String.format(
          "jCustos bootstrap requires both AuthenticationService "
              + "and AuthorizationService via SPI. Found: "
              + "AuthenticationService=%s, AuthorizationService=%s. "
              + "Check the @JCustosAutoService annotation processor "
              + "is wired in maven-compiler-plugin's "
              + "annotationProcessorPaths, and that "
              + "target/classes/META-INF/services/eu.jsentinel.jcustos."
    ...
    logger().info("jCustos runtime initialised:\n{}", runtime.log());
  }
}
